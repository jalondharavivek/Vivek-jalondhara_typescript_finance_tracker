...
import * as yup from "yup";
 import { newuser } from "../../../store/slice/User";
import { useDispatch, useSelector } from "react-redux";
import { registype } from "../../../utills/constants";

const Register = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const registerdata = useSelector((state : any) => state.users);
  ...
  const {
    ...
    formState : { errors  },
    reset,
  } = useForm<registype>({ resolver: yupResolver(userSchema) });

  const registeruser = (data : registype) => {
    let dataa = { ...data };

    const tralength = registerdata.reduce((ADDTRA : any) => ADDTRA + 1, 0);
